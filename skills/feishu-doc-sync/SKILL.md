...
description: Live workflow for validating and syncing local Markdown with Feishu cloud documents. Use when Codex needs to inspect Feishu auth and scopes, map `.md` files to Feishu docx documents, append or replace Markdown in tenant-visible docs, write back `feishu-index.json`, run directory pushes, or continue extending the bundled Feishu sync skill.
---

# Feishu Doc Sync

Use the bundled Python CLI to inspect prerequisites, validate live auth and docx connectivity, and execute the current tenant-mode Markdown sync path for Feishu document workflows. The current repo version can already fetch tenant tokens, create or inspect docs, read raw content, append Markdown, replace document body content, push one file, push a directory, and write back `feishu-index.json`.
...
- Validate live tenant auth or docx connectivity:
  Use `python scripts/feishu_doc_sync.py tenant-token`, `validate-tenant`, `create-document`, `get-document`, `get-raw-content`, `list-root-files`, or `delete-document`, then read [references/auth.md](./references/auth.md).
...
- Push a whole Markdown directory and update `feishu-index.json`:
  Use `python scripts/feishu_doc_sync.py push-dir <dir>`, then read [references/sync-rules.md](./references/sync-rules.md).
...
6. For one file, use `push-markdown`.
7. For one existing document body overwrite, use `replace-markdown` with `--confirm-replace`.
8. For one directory, use `push-dir`.
9. Extend media upload, pull/export, and user-mode sync only after the current tenant-mode mapping and permission model are stable.
...
- [references/markdown-mapping.md](./references/markdown-mapping.md): Markdown subset guidance, convert-to-block workflow, and media upload notes.
...
- `scripts/feishu_doc_sync.py`: CLI for auth validation, tenant-mode doc read/write operations, `push-markdown`, `push-dir`, and local planning flows.
